...
parser.add_argument(
    "--data_path",
    type=str,
    default="/home/user/dataset/sequences",
    help="raw data path",
)
parser.add_argument(
    "--processed_data_path", type=str, default="data/SemanticKITTI/dataset/sequences"
)
...
for seq_id in seq_list:
    ...
    if int(seq_id) < 11:
        ...
        for scan_id in scan_list:
            ...
            labels, sem_colors = load_label_kitti(
                join(label_path, str(scan_id[:-4]) + ".label"), remap_lut
            )
            labels = labels.squeeze()[:, None]
            write_ply(
                join(seq_path_out, scan_id)[:-4] + ".ply",
                [points[:, 0:3], sem_colors, labels, points[:, 3]],
