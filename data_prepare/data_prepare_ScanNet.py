...
parser.add_argument(
    "--data_path", type=str, default="/home/user/ScanNetv2", help="raw data path"
)
...
def handle_process(path):
    ...
    for line in lines:
        ...
        if line[0] == "axisAlignment":
            align_mat = (
                np.array([float(x) for x in line[2:]])
                .reshape([4, 4])
                .astype(np.float32)
            )
            R = align_mat[:3, :3]
    ...
    write_ply(
        str(out_f),
        [coords.astype(np.float64), colors, label[:, None].astype(np.float64)],
